...
public class Repeat{
	public static void main (String[]args){
		...
		String[] fruitsArray = fruits.split("/", 0);	// スラッシュ区切りで配列に変換
		List<String> fruitsList = new ArrayList<>();	// 重複値を入れるリストの作成
		
		for (int i = 0; fruitsArray.length > i ; i++) {		// 配列の値を順番に取得(i)
			if (!fruitsList.contains(fruitsArray[i])) {		// 重複リストに配列変数の要素がないなら、for文へ、
				for (int num = i + 1; fruitsArray.length > num; num++) {	// 配列の値を順番に取得(num)
