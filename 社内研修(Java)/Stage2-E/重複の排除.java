...
public class Repeat{
	public static void main (String[]args){
		...
		for (int i = 0; fruitsArray.length > i ; i++) {		    // 配列変数の要素数がiより大きいときにif文を繰り返す
		if (!fruitsList.contains(fruitsArray[i])) {				// 重複リストに配列変数の要素がないなら、for文へ、
		for (int num = i + 1; fruitsArray.length > num; num++) {	// 配列変数の要素数がnumより大きいときにif文を繰り返す
		...
		System.out.println("1:" + fruitsList.get(1));
        System.out.println("2:" + fruitsList.get(0));
		}
	}